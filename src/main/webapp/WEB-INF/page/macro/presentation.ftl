...
<tr>
		...
		<tr>
			...
			<td nowrap="nowrap" width="210">
				<img class="icon_vote_lcap" src="${contextPath}/images/transp.gif" alt=""/><img
					src="${contextPath}/templates/${templateName}/images/voting_bar.gif"
					width="${option.votePercentage * 2}" height="12" alt=""/><img class="icon_vote_rcap"
					src="${contextPath}/images/transp.gif" alt=""/>
			</td>
	...
	<tr>
		<td colspan="4" class="strong"
				align="center">${I18n.getMessage("PostShow.pollTotalVotes")} ${poll.totalVotes}</td>
	</tr>
</table>
</#macro>
...
<#macro moderationButtons>
	<#if moderator  && openModeration?default(false)>
		<#if can_remove_posts?default(false)><input type="submit" name="topicRemove"
				value="&nbsp;&nbsp;${I18n.getMessage("Delete")}&nbsp;&nbsp;" class="liteoption"
				onclick="return validateModerationDelete();"/></#if>
		<#if can_move_topics?default(false)><input type="submit" name="topicMove"
				value="&nbsp;&nbsp;${I18n.getMessage("move")}&nbsp;&nbsp;" class="liteoption"
				onclick="return verifyModerationCheckedTopics();"/></#if>
		<#if can_lockUnlock_topics?default(false)>
		<input type="submit" name="topicLock" value="&nbsp;&nbsp;${I18n.getMessage("Lock")}&nbsp;&nbsp;"
				class="liteoption" onclick="return lockUnlock();"/>			<input type="submit" name="topicUnlock"
				value="&nbsp;&nbsp;${I18n.getMessage("Unlock")}&nbsp;&nbsp;" class="liteoption"
				onclick="return lockUnlock();"/>
		</#if>
	</#if>
</#macro>
...
<script type="text/javascript">
	function todo(name) {
		var todo = document.getElementById("moderationTodo");
		todo.name = name;
		todo.value = "1";
	}
	...
	<#if isModerator || isAdmin>
		...
		<a href="javascript:deleteTopic();"><img class="icon_topic_delete" src="${contextPath}/images/transp.gif"
				title="${I18n.getMessage("Delete")}" alt=""/></a>
		</#if>

		<#if can_move_topics?default(false)>
		<a href="javascript:moveTopic();"><img class="icon_topic_move" src="${contextPath}/images/transp.gif"
				title="${I18n.getMessage("move")}" alt=""/></a>
		</#if>

		<#if can_lockUnlock_topics?default(false)>
			<#if topic.status == STATUS_LOCKED>
			<a href="javascript:lockUnlock(false);"><img class="icon_topic_unlock"
					src="${contextPath}/images/transp.gif" title="${I18n.getMessage("Unlock")}" alt=""/></a>
			<#else>
			<a href="javascript:lockUnlock(true);"><img class="icon_topic_lock" src="${contextPath}/images/transp.gif"
					title="${I18n.getMessage("Lock")}" alt=""/></a>
			</#if>
		</#if>
	</#if>
</#macro>
	...
	<tr>
		<td nowrap="nowrap">
			<form action="" name="f" id="f" accept-charset="${encoding}">
				...
				<select onchange="if(this.options[this.selectedIndex].value != -1){ document.location = '${contextPath}/forums/show/'+ this.options[this.selectedIndex].value +'${extension}'; }"
						name="select">
